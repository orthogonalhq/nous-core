    ...
    <Menubar.Menu>
      ...
      <Menubar.Portal>
        <Menubar.Content style={contentStyle} align="start" sideOffset={0}>
          <Item label="New Window" shortcut="Ctrl+Shift+N" onSelect={() => window.open(window.location.origin, '_blank')} />
          <Separator />
          <Item label="Close Tab" shortcut="Ctrl+W" onSelect={() => window.close()} />
        </Menubar.Content>
      </Menubar.Portal>
    </Menubar.Menu>
  )
