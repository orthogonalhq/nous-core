/**
 * WR-162 SP 10 — renderer-safe stub for `node:crypto`.
 *
 ...
 *
 * Vite/Rollup cannot tree-shake the top-level `import { randomUUID,
 * createHash } from 'node:crypto'` when bundling for the browser, and
 * Rollup's default `__vite-browser-external` stub does not export those
 * symbols. This shim provides browser-safe replacements so the renderer
 * bundle resolves; any accidental call into a proof-issuance helper from
 * the renderer would throw at call time, surfacing the contract defect
 * rather than silently producing an invalid proof.
 */
