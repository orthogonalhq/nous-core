...
type _ProviderVendorKeyIsExact = Expect<
  Equal<ProviderVendorKey, 'anthropic' | 'cloudflare-workers-ai' | 'codex-cli' | 'github-copilot-cli' | 'groq' | 'llama-cpp' | 'openai' | 'ollama'>
>;
type _BootstrapProviderKeyIsExact = Expect<
  Equal<BootstrapProviderKey, 'anthropic' | 'cloudflare-workers-ai' | 'codex-cli' | 'github-copilot-cli' | 'groq' | 'llama-cpp' | 'openai' | 'ollama'>
>;
...
describe('provider definition type derivation', () => {
  it('derives runtime roster keys from the production tuple', () => {
    ...
    expect(keys.sort()).toEqual(['anthropic', 'cloudflare-workers-ai', 'codex-cli', 'github-copilot-cli', 'groq', 'llama-cpp', 'ollama', 'openai']);
  });
