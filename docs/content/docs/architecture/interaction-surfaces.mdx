...
This surface is not a second control plane. The bridge app normalizes connector-native messages into canonical communication-gateway envelopes, then hands them to `ICommunicationGatewayService` inside the same app-hosted runtime graph. The bridge adapter stays thin; routing, binding, acknowledgement, delivery truth, and advisory continuity remain canonical runtime concerns.
