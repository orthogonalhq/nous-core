...
- Supports text in Phase 11.1; voice remains a later-phase escalation surface
...
This surface is not a second control plane. The bridge app normalizes connector-native messages into canonical communication-gateway envelopes, then hands them to `ICommunicationGatewayService` inside the same app-hosted runtime graph. Device identity, manual pairing, immutable endpoint direction, capability grants, signed transport validation, and incident containment flow through `IEndpointTrustService`; the bridge adapter stays thin and does not own trust state.
