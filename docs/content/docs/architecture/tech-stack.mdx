...
| **Build — application packages** | Next.js (`next build`) for web; tsc for CLI. tsdown re-evaluated for future app packages. |
...
Type-definition-heavy packages (shared types, interfaces, schemas, error classes) are built with **tsc**. The web app uses Next.js build; the CLI uses tsc. tsdown was deferred due to pnpm/Windows compatibility; it may be re-evaluated for future application packages. See ADR-001 for the rationale.
...
Monorepo:        pnpm workspaces
Build:           tsc (type packages, CLI) / Next.js (web app)
