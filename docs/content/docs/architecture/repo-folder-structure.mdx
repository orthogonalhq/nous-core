...
The compression engine. Implemented as `@nous/memory-distillation` (Phase 4.3). The package and heuristic baseline exist today, and Phase 8.4 adds a prototype evaluation harness for candidate distillation approaches. Production background promotion and supersession through this path remain gated until Phase 8.5 ratifies the runtime coupling.
...
This is the intended "sleep cycle" — consolidation that happens offline, not at query time, once the production distillation path is ratified.
