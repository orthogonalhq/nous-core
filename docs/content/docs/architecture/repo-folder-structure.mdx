...
```
providers/
├── adapters/                         # Provider adapter module objects
├── definitions/                      # Static provider metadata catalog
...
The static typed provider definition catalog lives in `self/subcortex/providers/src/definitions/`. The catalog contains metadata-only definitions for Anthropic, Chat Completions-compatible remote providers, and Ollama, and exports literal-derived provider key types. Provider adapter modules live in `self/subcortex/providers/src/adapters/`. Runtime bootstrap and credential loading still use the current provider-registration paths until the catalog is consumed by bootstrap.
