...
├── provider-adapters.ts               # Generated public concrete adapter export surface
├── provider-definitions.ts            # Generated static provider metadata catalog
├── provider-factories.ts              # Generated public provider factory catalog
├── providers/                         # Provider leaves with metadata, providers, and leaf-owned adapters
├── protocols/                         # Shared protocol adapters and transports
├── schemas/                           # Hand-written provider, adapter, and factory contracts
├── shared/                            # Shared output helpers and fallback adapters
...
The static typed provider definition catalog is generated directly into `self/subcortex/providers/src/provider-definitions.ts`; each provider metadata constant lives with its owning provider module. The catalog contains metadata-only definitions for Anthropic, Chat Completions-compatible remote providers, and Ollama, and exports literal-derived provider key types. Concrete provider-owned adapter modules live in their provider leaves, such as `self/subcortex/providers/src/providers/anthropic/adapter.ts` and `self/subcortex/providers/src/providers/ollama/adapter.ts`. `self/subcortex/providers/src/provider-adapters.ts` owns the generated public concrete-adapter export surface, and `provider-factories.ts` owns the generated provider factory catalog. Hand-written provider, adapter, and factory contracts live in `self/subcortex/providers/src/schemas/`; parsed-output helpers and fallback adapters live in `self/subcortex/providers/src/shared/`.
