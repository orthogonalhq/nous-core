...
The structure is organized by the Mind Model — four layers of consciousness that mirror the architecture of the human mind. See [The Mind Model](/docs/architecture/the-mind-model) for the philosophical foundation.
...
* Calls the PFC engine
* Coordinates the subcortex through interfaces
* Coordinates memory through interfaces
* Persists execution traces

Core is the bridge between consciousness and the layers below. It does not make decisions — it coordinates the flow that allows the PFC to make decisions.
...
Memory is its own layer because it is neither consciousness nor subconscious. It receives experiences from above (through PFC-approved writes) and serves context upward (through retrieval). It has internal processing (distillation) that runs on its own schedule.
...
The compression engine. Background process that converts clusters of experience records into distilled patterns.

Responsibilities:

* Identify clusters of related experience records
* Compress clusters into pattern statements
* Assign confidence scores based on consistency and volume
* Mark superseded records for archival
* Trigger meta-vector and relationship updates via knowledge-index
...
* Score candidates by semantic similarity, sentiment weight, recency, and confidence
* Enforce retrieval token budget
...
Cross-project intelligence. The layer that makes cross-referencing cheap at query time.

**meta-vectors/** — One embedding per project summarizing its domain, themes, and knowledge profile. Updated when distilled patterns are created. Enables fast project-level similarity search.

**taxonomy/** — Topic tags assigned to projects. Structural links between projects based on shared domains. Zero inference cost — pure set intersection.

**relationships/** — Background relationship graph maintained by periodic LLM evaluation. Maps validated knowledge links between projects. Amortizes expensive inference over time rather than paying it per query.

**discovery/** — Query-time orchestrator. Combines meta-vector search, taxonomy links, and relationship graph to determine which projects are relevant to the current situation. Fast and cheap.
...
├── escalation/                       # Human notification channels
...
Tools execute only through PFC authorization.
...
Multi-channel human notification.
...
See [Packages and Plugins](/docs/architecture/packages-and-plugins) for the full design.
...
14. `cortex/pfc/` — Prefrontal Cortex engine
...
22. `subcortex/escalation/` — human notification
