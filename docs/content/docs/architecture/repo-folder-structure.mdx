...
├── web/                              # Chat UI + Projects UI + Dashboard + MAO + mobile operations projection
...
* Bridge runtime that composes `ICommunicationGatewayService` in process
...
Phase 12.1 adds `self/cortex/core/src/agent-gateway/` as the canonical internal agent execution harness. The legacy `CoreExecutor` remains in place during the migration window, callers are not cut over in this phase, and workflow run-state authority remains in the Phase 9 workflow runtime.
...
Phase 12.3 adds `self/cortex/core/src/gateway-runtime/` as the boot assembly for the long-lived Principal/System pair. This folder owns deterministic boot sequencing, the Principal communication-tool overlay, the read-only System-context replica, runtime health snapshots, and the gateway-backed ingress adapter that forwards accepted work into System ownership. `CoreExecutor` still remains the active caller path during this migration step.
