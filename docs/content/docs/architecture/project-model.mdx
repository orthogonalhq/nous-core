...
- PFC enforces the protocol and routes within it
...
- PFC has high autonomy within declared guardrails
...
The user builds or approves a workflow graph. Each node represents a discrete step — a model call, a tool execution, a quality gate, a human decision point, a branching condition.
...
- **Type** — model call, tool execution, quality gate, human decision, condition, transform, etc.
...
- **Escalation rules** — when and how to involve the human
...
Hard gate. Binary pass/fail. No PFC discretion.

- The PFC cannot override a MUST gate
...
Strong default. PFC can deviate with justification.

- The PFC follows the defined behavior by default
- Deviation is permitted but must include a logged justification
- Deviations are flagged for human review (configurable)
- Used for: best practices, standard operating procedures, recommended defaults

Example: Video clips should be 5–10 seconds. The PFC may produce a 12-second clip if the content warrants it, but the deviation is logged and flagged.

### MAY

Discretionary. PFC uses judgment. Escalates only on low confidence.

- The PFC operates with full autonomy within the node's declared constraints
- No justification required for choices made
- Escalation occurs only when PFC confidence drops below a configured threshold
- Used for: creative decisions, optimization choices, non-critical parameter selection

Example: Wardrobe selection for a photo batch. The PFC chooses based on content calendar context and prior performance. No escalation unless the choice falls outside declared constraints.
...
## PFC as Project Supervisor

The PFC's role extends beyond per-turn evaluation. In the project model, the PFC is a persistent supervisor for the entire project lifecycle.
...
- Escalate to human through configured channels when required
- Propose protocol formalizations in intent projects
- Learn from outcomes to improve future decisions within the project

The PFC tier determines how much autonomous reasoning the supervisor applies. Protocol projects with explicit graphs may run at lower tiers. Intent projects with emergent workflows require higher tiers.

---

## Human Escalation

Human involvement is a first-class concept, not an edge case.

### Escalation Triggers

- A MUST gate fails
- A SHOULD deviation is flagged for review
- PFC confidence drops below the configured threshold on a MAY node
- An intent project encounters ambiguity it cannot resolve
- A workflow reaches a node explicitly typed as "Human Decision"
...
- What it needs from the human (decision, approval, clarification)
...
4. **Execute** — Nous PFC supervises execution. Workflow runs. Escalations occur as needed.
...
- **PFC Engine** → Gains a per-project supervisor mode in addition to per-turn evaluation
- **ModelRouter** → Must route heterogeneous provider types, not just text model roles
- **Tool Interface** → Nodes in the workflow are a superset of tools — they include model calls, quality gates, human decisions, and transforms alongside tool executions
...
- **Escalation Service** — multi-channel human notification with response tracking
