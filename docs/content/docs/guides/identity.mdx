...
Press the wizard's outer **Back** button at any sub-stage to return to **Welcome**. The Identity step does not have its own internal back navigation between sub-stages — back from A, B, or C lands you at Welcome the same way back from any other top-level step would. If you re-enter Identity later, the internal state machine starts at Sub-stage A again; nothing is held over.
