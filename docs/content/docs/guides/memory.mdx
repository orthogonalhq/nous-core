...
**Project discovery (Phase 6.2):** Cross-project retrieval can be filtered by project relevance. The Knowledge Index provides:
...
Discovery produces candidate `targetProjectIds`. The caller must apply policy filter before passing them to retrieval. All retrieval still flows through `PolicyEnforcedRetrievalEngine`; discovery does not bypass policy.
