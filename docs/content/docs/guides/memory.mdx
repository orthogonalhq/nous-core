...
Experience records can be compressed into **distilled patterns** — consolidated knowledge derived from clusters of related experiences. `@nous/memory-distillation` owns that model, and Phase 8.4 adds the prototype evaluation harness used to judge candidate distillation approaches before they are allowed to mutate live project memory.

Current runtime posture: production background promotion and supersession through this path are still gated for Phase 8.5. The rest of this section describes the intended distillation model and the prototype criteria being validated, not an already-live autonomous background loop in operator-facing memory surfaces.
...
**Confidence lifecycle target:** Patterns should gain confidence when new experience records align (refresh); they should lose confidence when contradicted or when they become stale (decay). Phase 8.4 explicitly tests contradiction and staleness behavior before that lifecycle is promoted into the production runtime.
...
**Explainability** — When a pattern is eventually allowed to influence an outcome, the system must produce a trace-linked explanation (what pattern, which outcome, evidence refs) for audit and operator inspection. Phase 8.4 keeps this as a prototype gate requirement rather than a silently enabled production behavior.
