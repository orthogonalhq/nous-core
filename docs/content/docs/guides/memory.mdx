...
**Confidence lifecycle:** Patterns gain confidence when new experience records align (refresh); they lose confidence when contradicted or when they become stale (decay). Phase 8.5 implements deterministic initial assignment, confirming refresh, contradiction decay, staleness decay, and retirement flagging while preserving explicit confidence metadata for downstream governance and visibility consumers.
...
**Explainability** — When a pattern is allowed to influence an outcome, the system must produce a trace-linked explanation (what pattern, which outcome, evidence refs) for audit and operator inspection. Phase 8.5 emits the evidence-linked pattern, confidence, escalation, and export payloads required for that path; later phases decide when those outputs drive autonomy or operator-facing UI surfaces.
