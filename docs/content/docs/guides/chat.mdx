...
The first time you open the principal chat panel after finishing the first-run wizard, the agent emits a single greeting message. The welcome is part of the [first-run experience](/docs/getting-started/first-run) — the wizard configures the agent's identity, then the chat introduces it.
...
The welcome composes through the same prompt path as every chat turn, so anything that flows into a normal agent reply also flows into the welcome — your chosen agent name, the personality preset (or any individual trait overrides), the user profile context you optionally shared on the Identity step. There is no separate "welcome template"; the agent simply produces a first turn under the same composition contract.
