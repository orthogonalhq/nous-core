...
The name is deliberate. *Nous* is Greek for "mind" — the faculty of intellectual apprehension, the highest form of thinking. The architecture is modeled on the structure of human consciousness. See [The Mind Model](/.architecture/the-mind-model).
...
1. **The Skeleton** — `self/` structure, interfaces, web chat UI, basic routing, basic PFC
...
→ See: [Roadmap Overview](/.architecture/roadmap) (engineering roadmap — in `.architecture/` submodule)
