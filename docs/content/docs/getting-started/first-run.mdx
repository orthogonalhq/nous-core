...
The validation check runs **once at step mount**, not on every keystroke or click. Nous keeps a 30-minute per-spec session cache so navigating back to the step does not re-issue probes.
